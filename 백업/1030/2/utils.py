...
class CTCLabelConverter(object):
    ...
    def encode(self, text, batch_max_length=25):
        ...
        print('encode  함수 들어옴 ')
        print(f'encode  함수 들어옴 text : {text}')

        # The index used for padding (=0) would not affect the CTC loss calculation.
        batch_text = torch.LongTensor(len(text), batch_max_length).fill_(0)
        print(f'encode  함수 들어옴 22222 list(t)  text : {text}')
        print(f'encode  함수 들어옴 22222 list(t)  batch_text : {batch_text}')
        for key, value in self.dict.items():
            print(f"@@@@@@@@@@@@@@@@encode  함수 들어옴 Key: {key}, Value: {value}")
        ...
        for i, t in enumerate(text):
            ...
            print(f'encode  11111111111111111111111111함수 들어옴 22222 i    : {i}')
            print(f'encode 2222222222222222222 함수 들어옴 22222 t    : {t}')

            text = list(t)


            print(f'encode 3333333333333333333333 함수 들어옴 22222    text : {text}')

        #     batch_text[i][:len(text)] = torch.LongTensor(text)
        # return (batch_text.to(device), torch.IntTensor(length).to(device))

            #text = [self.dict[char] for char in text]
            #print(f'encode  함수 들어옴 22222 torch.LongTensor(text) : {torch.LongTensor(text)}')
            #print(f'encode  함수 들어옴 22222 batch_text: {batch_text)}')

            #batch_text[i][:len(text)] = torch.LongTensor(text)
            #print(f'encode 3333333333333333333333 함수 들어옴 22222    text : {torch.IntTensor([int(char) for char in text])}')

            #batch_text[i][:len(text)] = torch.IntTensor([int(char) for char in text])

            # # 문자를 정수로 매핑하는 딕셔너리 생성
            # char_to_int = {char: i for i, char in enumerate(text)}
            #
            # # 문자열을 정수 목록으로 변환
            # int_list = [char_to_int[char] for char in text]
            #
            # # 정수 목록을 텐서로 변환
            # tensor = torch.LongTensor(int_list)
            #
            # print(int_list)  # [0, 1, 2, 2, 3]
            # print(tensor)  # tensor([0, 1, 2, 2, 3])
            # print('******************************************************')
            ...
            #
            # # 정수 목록을 텐서로 변환
            # tensor = torch.tensor(int_list, dtype=torch.int64)  # 또는 torch.tensor(int_list, dtype=torch.int32)
            #
            # print(int_list)  # [0, 1, 2, 2, 3]
            # print(tensor)  # tensor([0, 1, 2, 2, 3])
            # print('******************************************************')
        ...
        #     batch_text[i][:len(text)] = torch.IntTensor([int(char) for char in text])
            ...
            #for i, t in enumerate(text):
                ...
                # print(f'i : {i}')
                # print(f't : {t}')
                # #int_list = [int(char) for char in text]
                #
                # #text = "สวัสดีครับ"  # 태국어 문자열
                #
