    ...
    for dirpath, dirnames, filenames in os.walk(root+'/'):
        if not dirnames:
            ...
            if select_flag:
                ...
                sub_dataset_log = f'죽겠네 진짜 ㅜㅜㅜㅜ sub-directory:\t/{os.path.relpath(dirpath, root)}\t num samples: {len(dataset)}'
...
class LmdbDataset(Dataset):
    ...
    def __init__(self, root, opt):
        ...
        print(f'LmdbDataset 함수 lmdb 초기화 해야 함')
        self.root = root
        self.opt = opt
        ...
        with self.env.begin(write=False) as txn:
            print('진짜 어렵다 ㅜㅜㅜㅜㅜㅜㅜ')
            print(f'LmdbDataset root 경로 : {self.root}')
            import lmdb
            import os
            from PIL import Image
            ...
            try:
                with env.begin(write=False) as txn:
                    ...
                    for index in range(self.nSamples):
                        ...
                        label_key = 'label-image_%01d.png'.encode() % index
                    ...
                    if self.opt.data_filtering_off:
                        ...
                        for index in range(self.nSamples):
                            ...
                            #label_key = 'label-%09d'.encode() % index
                            label_key = 'label-image_%01d.png'.encode() % index
                            print(f'txn.get(label_key) : {txn.get(label_key)}')
                            label = txn.get(label_key).decode('utf-8')
