...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_iter', type=int, default=30, help='number of iterations to train for')
    #parser.add_argument('--valInterval', type=int, default=2000, help='Interval between each validation')
    parser.add_argument('--valInterval', type=int, default=30, help='Interval between each validation')
    ...
    #(f'==============================!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!인코딩 하기전은 물음표임?????? opt_character : {opt_character}')
    # if 'ๆ' in opt_character:
    #     # 'ๆ' 문자가 있을 경우
    #     #print('==============================!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!인코딩 하기전은 물음표임??????  있음')
    # else:
    #     # 'ๆ' 문자가 없을 경우
    #     #print('==============================!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!인코딩 하기전은 물음표임??????  없음')
    #
    # #encoded_text = opt_character.encode("utf-8")  # 다른 인코딩으로 변환하려면 해당 인코딩을 지정
    # #3print(f'==============================!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!인코딩 잘 됨????? encoded_text : {encoded_text}')
    ...
    opt_character = ' ()+-./01234569ACDFGILQSTWabeghimnoprstuz~°กขคงจฉชซฑณดตถทธนบปผฝพฟภมยรลวศษสหฬอะัาำิีึืุูเแโใไๆ็่้๊์✓'
    #print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!opt_character 인코드 하기 전 날것 : {opt_character}')
    # if 'ๆ' in opt_character:
    #     # 'ๆ' 문자가 있을 경우
    #     #print('==============================!!!!!!!!!!!!!!!!!!!!!!!!!opt_character 인코드 하기 전 날것  있음')
    # else:
    #     # 'ๆ' 문자가 없을 경우
    #     #print('==============================!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!opt_character 인코드 하기 전 날것  없음')
    encoded_text = opt_character.encode("utf-8").decode("utf-8")
    #print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!아규먼트에 넣기전 인코드 했다가 다시 디코드 함 encoded_text : {encoded_text}')
    # if 'ๆ' in encoded_text:
    #     # 'ๆ' 문자가 있을 경우
    #     #print('==============================!!!!!!!!!!!!!!!!!!!!!!!!!아규먼트에 넣기전 인코드 했다가 다시 디코드 함 있음')
    # else:
    #     # 'ๆ' 문자가 없을 경우
    #     #print('==============================!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!아규먼트에 넣기전 인코드 했다가 다시 디코드 함  없음')
    # #parser.add_argument('--character', type=str, default=encoded_text, help='character label')


    opt_character = ' ()+-./01234569ACDFGILQSTWabeghimnoprstuz~°กขคงจฉชซฑณดตถทธนบปผฝพฟภมยรลวศษสหฬอะัาำิีึืุูเแโใไ็็่้๊์✓'.encode(
        'utf-8').decode('utf-8')
    opt_character = ' ()+-./01234569ACDFGILQSTWabeghimnoprstuz~°กขคงจฉชซฑณดตถทธนบปผฝพฟภมยรลวศษสหฬอะัาำิีึืุูเแโใไๆ็่้๊์✓'.encode(
        'utf-8').decode('utf-8')
    #print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!parser.add_argument 함수에 들어가기 전 인코드 했다가 디코드 opt_character : {opt_character}')
    # if 'ๆ' in opt_character:
    #     # 'ๆ' 문자가 있을 경우
    #     #print('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!parser.add_argument 함수에 들어가기 전 인코드 했다가 디코드 있음')
    # else:
    #     # 'ๆ' 문자가 없을 경우
    #     #print('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!parser.add_argument 함수에 들어가기 전  인코드 했다가 디코드 없음')
    ...
    #parser.add_argument('--character', type=str, default=opt_character, help='character label')
    #print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!sys.setdefaultencoding utf-8 로 바꿈!!!!!!!!!!!!!!!!  opt_character : {opt_character}')
    # if 'ๆ' in opt_character:
    #     # 'ๆ' 문자가 있을 경우
    #     #print('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!인코드 디코드 있음')
    # else:
    ...
    # parser.add_argument('--character', type=str,
    #                     default=' ()+-./01234569ACDFGILQSTWabeghimnoprstuz~°กขคงจฉชซฑณดตถทธนบปผฝพฟภมยรลวศษสหฬอะัาำิีึืุูเแโใไ็็่้๊์✓',
    #                     help='character label')
    #print('미세하게 y 처럼 생긴게 합쳐지면서 없어져 버림 을 수정해서 제대로 하드코딩 함')
    parser.add_argument('--character', type=str,
