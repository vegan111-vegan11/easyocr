...
def train(opt):
    ...
    for root, dirs, files in os.walk(path_to_weights):
        for file in files:
            ...
            if "best_prediction_accuracy_" in file:
                ...
                current_prediction_accuracy = connected_parts.split("_")[-1].split("_")[0]
    ...
    opt.FT = True
    print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!saved_model : {opt.saved_model}')
    ...
    print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!valid_loader 밸리드는 왜 71 로 주냐고 ㅜㅜㅜㅜ 줄지마 ㅜㅜㅜㅜㅜlen(valid_dataset) : {len(valid_dataset)}')
    ...
    while (True):
        ...
        if ( iteration + 1) % opt.valInterval == 0 or iteration == 0:  # To see training progress, we also conduct validation when 'iteration == 0'
            ...
            with open(f'./saved_models/{opt.exp_name}/{current_date}/log_train_{current_time}.txt', 'a', encoding='utf-8') as log:
                # 나머지 코드
                ...
                if current_accuracy > best_accuracy:
                    ...
                    # torch.save(model.state_dict(),
                    #            f'./saved_models/{opt.exp_name}/{current_date}/best_accuracy_{current_time}_{current_accuracy:.4f}.pth')
                    torch.save(model.state_dict(),
                ...
                if current_norm_ED > best_norm_ED:
                    ...
                    # torch.save(model.state_dict(),
                    #            f'./saved_models/{opt.exp_name}/{current_date}/best_norm_ED_{current_time}_{current_norm_ED:.4f}.pth')
                    torch.save(model.state_dict(),
                ...
                print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!정확도 계산 accuracy : {accuracy}')

                print('!!!!!!! 실제 맞은게 많은 걸로 저장하는게 더 좋지 않을까')
                ...
                if current_prediction_accuracy > best_prediction_accuracy:
                    ...
                    torch.save(model.state_dict(),
                               f'./saved_models/{opt.exp_name}/{current_date}/best_prediction_accuracy_{current_time}_{current_accuracy:.4f}_{iteration + 1}.pth')
                ...
                accuracy_log = f'{"Accuracy":17s}: {accuracy:0.3f}\n{"correct_predictions_num":17s}: {sum(correct_predictions)}\n{"predictions_num":17s}: {len(correct_predictions)}'
                print(accuracy_log)
        ...
        if (iteration + 1) % (opt.num_iter // 2) == 0:
            print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!iter 마다 주기적으로 가중치 저장 시작 (num_iter // 2) : {(opt.num_iter // 2)}')
            torch.save(
        ...
        if (iteration + 1) == opt.num_iter:
            print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!iter + 1 이랑  opt.num_iter 랑 같으면 트레인 종료한다 iteration 에 1 더한 후: {iteration}')
            print(
