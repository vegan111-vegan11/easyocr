...
def train(opt):
    ...
    path_to_weights = r"C:\Users\TAMSystech\yjh\ipynb\deep-text-recognition-benchmark\saved_models\None-VGG-BiLSTM-CTC-Seed1111"
    best_accuracy = -1
    current_accuracy_list = []
    for root, dirs, files in os.walk(path_to_weights):
        for file in files:
            if "best_accuracy_" in file:
                ...
                current_accuracy_list.append(float(current_accuracy))
                # 파일 이름 부분을 "_"으로 분할하여 가장 마지막 요소 추출


    print(f'!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!current_accuracy_list : {current_accuracy_list}')
    max_accuracy = max(current_accuracy_list)
