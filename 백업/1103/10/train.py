...
def train(opt):
    ...
    while (True):
        ...
        # 원래 파일 경로 및 파일명
        original_log_file_name = f"./saved_models/{opt.exp_name}/{current_date}/log_train_{current_time}.txt"


        # 추가할 문자열
        suffix = f"_{accuracy}_{iteration + 1}.txt"

        # 새로운 파일 경로 및 파일명 생성
        new_log_file_name = original_log_file_name.replace('.txt', suffix)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_iter', type=int, default=5, help='number of iterations to train for')
